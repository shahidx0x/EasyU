...
function Booking() {
  ...
  const [fsm] = useGetFSM();
  const filteredWorkshop = fsm
    ?.filter((workshop) => !workshop.usr.includes("Filling Station"))
    .map((workshop) => ({
      img: workshop.img,
      name: workshop.usr,
      ...
      <div className="flex flex-col h-full p-3 w-80 dark:bg-gray-900 dark:text-gray-100">
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <h2 className="font-bold">Find Nearby Workshop / Petrol Pump</h2>
          </div>
              ...
              >
                <div className="bg-gray-50">
                  <div className="flex gap-4">
                    ...
                    />
                    <div>
                      <h3 className="font-bold text-2xl">{m.name}</h3>
                      <div className="flex gap-1">
                        <BiPhoneCall className="text-xl mt-1" />
                        <p className="font-mono text-lg">0{m.num || "0181"}</p>
                      </div>
                      <button className="w-20 rounded-sm btn-outline btn-primary border">
                        <Link
                          to={`/make/booking/${m.name}/${m.num || "0181"}/${
                            m.mail || "t@t.com"
                          }`}
                        >
                          Make Booking
