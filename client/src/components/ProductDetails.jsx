...
const ProductDetails = () => {
  ...
  const { id, name, cat, price, image } = useParams();
  const imgStr = image.replaceAll("[", "/");
  const addCount = () => {
  ...
  const addCartInfoToServer = (name, price, mail, image, des, quan) => {
    const payload = { name, price, mail, image, des, quan };
    console.log(payload);
    axios
      .post("http://localhost:5000/api-v1/add/carts", payload)
      .then((res) => console.log(res.status));
    navigate("/customer/my-carts");
  };
      ...
      <div className="flex justify-center items-center lg:flex-row flex-col gap-8">
        ...
        <div className="  w-full sm:w-96 md:w-8/12 lg:w-6/12 items-center">
          ...
          <div className="lg:mt-11 mt-10">
            ...
            <p className="text-xs text-gray-600 px-2 bg-gray-200 py-2 rounded-md w-44">
              {cat}
            </p>
          ...
          <button
            onClick={() =>
              addCartInfoToServer(name, price, user?.email, imgStr, cat, count)
            }
            className=" focus:outline-none focus:ring-2 hover:bg-black focus:ring-offset-2 focus:ring-gray-800 font-medium text-base leading-4 text-white bg-indigo-700 w-full py-5 lg:mt-12 mt-6"
          >
            Add to cart
          </button>
        </div>
        ...
        <div className=" w-full sm:w-96 md:w-8/12  lg:w-6/12 flex lg:flex-row flex-col lg:gap-8 sm:gap-6 gap-4">
          <div className=" w-full lg:w-8/12 bg-gray-100 flex justify-center items-center">
            <img src={imgStr} alt="Wooden Chair Previw" />
          </div>
          <div className=" w-full lg:w-4/12 grid lg:grid-cols-1 sm:grid-cols-4 grid-cols-2 gap-6">
            <div className="bg-gray-100 flex justify-center items-center py-4">
              <img src={imgStr} alt="Wooden chair - preview 1" />
            </div>
            <div className="bg-gray-100 flex justify-center items-center py-4">
              <img src={imgStr} alt="Wooden chair - preview 2" />
            </div>
            <div className="bg-gray-100 flex justify-center items-center py-4">
              <img src={imgStr} alt="Wooden chair- preview 3" />
            </div>
          </div>
        </div>
      </div>
